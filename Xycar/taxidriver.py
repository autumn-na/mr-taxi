...
class TaxiDriver(object):
    ...
    def procPickup(self):
        if self.getData()['is_found'] == True and self.getData()['is_picked_up'] == True and self.getData()['dest'] != '' and self.drive == False and self.getData()['is_arrived'] == False:
            self.drive = True
    ...
    def procArrive(self, _frame):
        if self.getData()['is_found'] == True and self.getData()['is_picked_up'] == True and self.getData()['dest'] != '' and \
                self.getData()['is_arrived'] == False and self.getData()['dest'] == self.qr_reader.getQRText(_frame):
            self.cost = int((time.time() - self.pickup_time) * 1000) 
            self.drive = False
            self.arrive()
            print('Arrived!')
